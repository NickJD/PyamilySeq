...
def cluster(options):
    ...
    pangenome_clusters_PEP = OrderedDict()
    pangenome_clusters_PEP_SEQS = OrderedDict()
    ...
    cores = OrderedDict({'pep_core_99':0,'pep_core_95':0,'pep_core_15':0,'extended_99':0,'extended_95':0
    ...
    multi_PEP_Combined_By_StORFs = OrderedDict()
    multi_PEP_Combined_By_StORFs_1 = OrderedDict()
    ...
    Combined_pangenome_clusters_ONLY_StORF_Type = defaultdict(list)
    Combined_pangenome_clusters_StORF_Type = defaultdict(list)
    ...
    for clust, counts in pangenome_clusters_Type.items():
        if counts[1] >= len(unique_genomes):
            counts[1] = len(unique_genomes)
        ...
        if (counts[1] >=1 and counts[3] >=1):# and counts[0] >1 and counts[1] >1:
            if counts[1] >= len(unique_genomes):
                counts[1] = len(unique_genomes) # Not fake - it controls the combined cluster problem
    ...
    for i in range(len(unique_genomes)+1):
        if i > 1:
